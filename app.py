...
# --- 1. Initialize the ASR pipeline ---
# Set the device to use (GPU if available, otherwise CPU)
device = "cuda:0" if torch.cuda.is_available() else "cpu"

# Determine the appropriate data type for the model based on the device
# Use float16 for CUDA for better performance, otherwise float32
torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32

print("Initializing the ASR pipeline...")
...
with gr.Blocks() as iface:
    ...
    with gr.Row():
        with gr.Column():
            ...
            ref_text_input = gr.Textbox(label="Text in Reference Prompt Audio", placeholder="Transcription will appear here...", lines=2, interactive=False)
